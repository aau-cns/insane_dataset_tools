...
function [stamp, linear, angular] = read_geometry_twist_w_cov_msgs(bag,topic)
%READ_GEOMETRY_TWIST_W_COV_MSGS

disp('Reading Geometry Twist with Cov msgs...');
...
for k = 1:length(msgs)
    ...
    linear(:,k) = msgs{1,k}.twist.twist.linear(:);
    angular(:,k) = msgs{1,k}.twist.twist.angular(:);
end
end
